...
@tool(
    ...
    "when the customer's message asks about sizes, measurements or fit it attaches that product's "
    "size chart picture *instead of* the product photo -- a sizing answer carries the chart alone, "
    "and a product answer carries photos and no chart, unless the customer asked to see both. "
    "So a sizing reply says «ده جدول المقاسات», never «دي صورته». Either way it is attached for you: "
    ...
    "including 'send me all the colours', which is the case it exists for. One photo per colourway "
    "goes out only when the customer's own words asked for the colours; otherwise 'more' means up to "
    "two extra angles, unseen photos first and never repeating one already sent unless there is "
    "nothing else left to show. **One photo per product per reply is the rule otherwise**, so a "
    "customer who answers 'both' to 'photos of which one?' wants two photos and you get them by "
    "calling this once per product -- never by asking for a colour gallery of either. "
